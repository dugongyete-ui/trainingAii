...
def ensure_indonesian_system_prompt(messages):
    normalized = [dict(message) for message in messages]
    if normalized and normalized[0].get("role") == "system":
        normalized[0]["content"] = INDONESIAN_SYSTEM_PROMPT + "\n\n" + normalized[0].get("content", "")
    else:
        normalized.insert(0, {"role": "system", "content": INDONESIAN_SYSTEM_PROMPT})
    return normalized


def generate_stream_response(messages, temperature, top_p, max_tokens, tools=None, open_thinking=False):
    try:
        messages = ensure_indonesian_system_prompt(messages)
...
async def chat_completions(request: ChatRequest):
    try:
        messages = ensure_indonesian_system_prompt(request.messages)
        if request.stream:

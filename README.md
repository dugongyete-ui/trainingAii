...
menggunakan fondasi ringan **Qwen2.5-0.5B-Instruct** agar inference CPU lebih
realistis dari HP. Qwen2.5-3B-Instruct tetap didukung sebagai opsi model besar
jika seluruh checkpoint-nya tersedia.
...
cd minimind && bash install.sh
```

`install.sh` 会安装依赖、使用 CPU 版本的 PyTorch，并下载 checkpoint
multilingual Qwen yang dipakai untuk respons Bahasa Indonesia. Setelah selesai,
jalankan:

...
def main():
    ...
    parser.add_argument('--temperature', default=0.45, type=float, help="Temperatur generasi (lebih rendah menghasilkan jawaban lebih stabil)")
    ...
    input_mode = int(input('[0] Tes otomatis\n[1] Input manual\n'))
    ...
    for prompt in prompt_iter:
        ...
        history = conversation[-args.historys:] if args.historys else []
        conversation = [{"role": "system", "content": INDONESIAN_SYSTEM_PROMPT}] + history
        conversation.append({"role": "user", "content": prompt})
        if 'pretrain' in args.weight:
            inputs = tokenizer.bos_token + INDONESIAN_SYSTEM_PROMPT + "\n\nPengguna: " + prompt + "\nAsisten:"
        else:
